// Vercel Serverless Function: /api/paper-admin
// 职责：论文小助手 Q&A 问答「后台管理」接口（仅站长可用，X-Manage-Key 校验）
//   管理密钥与精选墙共用同一把 FEATURED_MANAGE_KEY，不新增环境变量
//   统一 POST 动作路由（极简版 Q&A 无 category 字段）
//     action=list            返回全部问答（含未发布），order: sort asc, id asc
//     action=create          新增问答（question/answer，排序自动置尾）
//     action=update          编辑问答（id + question/answer）
  ...
  try {
    ...
    if (action === 'list') {
      const rows = await fetchRows(
        '?select=id,question,answer,sort,published,created_at,updated_at&order=sort.asc,id.asc'
      );
