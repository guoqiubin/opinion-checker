...
- 简历助手（v2.10）：岗位科普后可上传可复制文字的 PDF / DOCX 通用简历，在浏览器本地解析后结合目标 JD 生成匹配优势、缺口、逐项修改建议和按 STAR 法则改写的参考简历；扫描版 PDF 第一版暂不支持，原始文件不上传，生成结果保存到当前浏览器
...
`POST /api/job-guide` 的个性化简历模式

```json
{ "mode": "resume-personalize", "jobTitle": "用户体验运营", "company": "拼多多", "jobDescription": "目标岗位 JD 文本", "resumeText": "浏览器本地解析出的简历文本" }
```

- `resumeText` 只接收浏览器解析后的文字，不接收原始 PDF / DOCX 文件
- 接口输出 JD 匹配总结、优势、缺口、逐项修改建议、待补充信息和参考简历
- 参考简历严格基于用户提供的事实，缺少数据的位置使用 `XX`，不虚构经历与结果
